...
test(string) :-
    json_parse("""this is a normal string""", string("this is a normal string")).

test(string_with_escape) :-
    json_parse("""this is a escaped\n string""", string("this is a escaped\n string")).

test(string_with_unicode_escape) :-
    json_parse("""this is a unicode escaped\x5C\u3AF6 string""", 
               string("this is a unicode escaped\x3AF6\ string")).

test(string_with_consective_unicode_chars) :-
    json_parse("""three-in-row\\u9AA5\\u01B7\\uFFFFd""", 
               string("three-in-row\x9AA5\\x01B7\\xFFFF\d")).
...
test(mixed_array) :-
    json_parse(" [ 5, ""yoga"", { ""name"" : ""james"", ""age"" : 25 }, [[]] ]", 
    [5, string("yoga"), obj([ pair(name, string("james")), pair(age, 25)]), [[]]]).
...
test(plain_object) :-
    json_parse("{\n\t""name"" : ""james"",\n\t""age"" : 25,\n\t""email"" : ""james.rodriguez@realmadrid.com"" }",
               obj([pair(name, string("james")), pair(age, 25),
                    pair(email, string("james.rodriguez@realmadrid.com"))])).

test(nested_object) :-
    json_parse("{\n\t""name"" : ""james rodr\\u00CDguez"",\n\t""age"" : 25.43E+1,\n\t""born"" : {\n\t\t""country"" : ""columbia"",\n\t\t""city"" : ""C\\u00DAcuta"",\n\t\t""Date"" : { ""Year"" : 1991,\n\t\t\t""month"" : ""July"",\n\t\t\t""Day"" : 12\n\t\t}\n\t}\n}",
               obj([pair(name, string("james rodr\x00CD\guez")),
               pair(age,2.543E+2),
               pair(born,obj([pair(country,string("columbia")),
               pair(city,string("C\x00DA\cuta")),
